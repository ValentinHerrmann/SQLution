from datetime import datetime
from io import BytesIO
import os
import re
import collections
import shutil
import zipfile
import time

from myapp.models import ZippedFolder


# Unterstützte Datentypen in SQLite
DATATYPE_MAP = {
    'int': 'INTEGER',
    'integer': 'INTEGER',
    'ganzzahl': 'INTEGER',
    'text': 'TEXT',
    'zeichenkette': 'TEXT',
    'varchar': 'TEXT',
    'string': 'TEXT',
    'float': 'REAL',
    'kommazahl': 'REAL',
    'double': 'REAL',
    'bool': 'BOOLEAN',
    'boolean': 'BOOLEAN',
    'wahrheitswert': 'BOOLEAN',
    # Weitere Typen können bei Bedarf ergänzt werden
}

def timestamp():
    return datetime.now().strftime("[%Y-%m-%d %H:%M:%S]") + "\t"

def parse_attribute(attr_str):
    """Zerlegt ein Attribut in Spaltenname und Datentyp"""
    if ':' in attr_str:
        name, dtype = [s.strip() for s in attr_str.split(':', 1)]
    else:
        parts = attr_str.strip().split(' ', 1)
        dtype, name = parts if len(parts) == 2 else (parts[0], parts[0])
    return name, DATATYPE_MAP.get(dtype.lower(), dtype.upper())

def parse_json_model(data: dict) -> tuple:
    if "model" in data: 
        data = data["model"]
    
    class_elements = {}
    attributes = {}
    
    for e in data["nodes"]:
        if e["type"].lower() == "class":
            class_elements[e["id"]] = e
            attrs = e['data']['attributes']
            for a in attrs:
                attributes[a['id']] = a
    
    foreign_keys_map, class_elements = parse_relationships(data, class_elements)
    pk_map = parse_primarykeys(class_elements, attributes, foreign_keys_map)
    
    return class_elements, attributes, foreign_keys_map, pk_map

def parse_relationships(data: dict, class_elements: dict) -> tuple:
    foreign_keys_map = {}
    for rel in data["edges"]:
        if rel["type"] == "ClassUnidirectional":
            foreign_keys_map = parse_uni_relationship(rel, foreign_keys_map)
        if rel["type"] == "ClassBidirectional":
            foreign_keys_map, class_elements = parse_bi_relationship(rel, foreign_keys_map, class_elements)
    return foreign_keys_map, class_elements

def parse_uni_relationship(relation: dict, foreign_keys_map: dict) -> dict:
    source = relation["source"]
    target = relation["target"]
    
    role = ''
    if 'targetRole' in relation['data']:
        role += relation["data"]["targetRole"]
    if 'sourceRole' in relation['data']:
        role += relation["data"]["sourceRole"]
    foreign_keys_map.setdefault(source, []).append((role, target))
    return foreign_keys_map

def parse_bi_relationship(relation: dict, foreign_keys_map: dict, class_elements: dict) -> tuple:
    source = relation["source"]
    target = relation["target"]
    role = ''
    if 'targetRole' in relation['data']:
        role += relation["data"]["targetRole"]
    if 'sourceRole' in relation['data']:
        role += relation["data"]["sourceRole"]

    class_elements[role+"_mn"] = {
        "id": role+"_mn",
        "name": role,
        "type": "Class",
        "data": {
            "name": role,
            "attributes": [],
        },
    }
    foreign_keys_map.setdefault(role+"_mn", []).append((class_elements[target]['data']["name"].lower()+'_id', target))
    foreign_keys_map.setdefault(role+"_mn", []).append((class_elements[source]['data']["name"].lower()+'_id', source))
    
    return foreign_keys_map,class_elements

def parse_primarykeys(class_elements: dict, attributes: dict, foreign_keys_map: dict) -> dict:
    pk_map = {}
    for class_id, class_data in class_elements.items():
        attr_entries = class_data.get("attributes", [])
        if attr_entries:
            first_attr_entry = attr_entries[0]
            first_attr = (
                first_attr_entry
                if isinstance(first_attr_entry, dict)
                else attributes.get(first_attr_entry)
            )
            if first_attr:
                pk_name, pk_type = parse_attribute(first_attr.get("name", "id"))
                pk_map[class_id] = (pk_name, pk_type)
        elif class_id.endswith("_mn"):
            pk1 = foreign_keys_map[class_id][0][0]
            pk2 = foreign_keys_map[class_id][1][0]
            pk_name = pk1 + "," + pk2
            pk_map[class_id] = (pk_name, "")
    return pk_map

def extract_tables(data: dict) -> str:
    try:
        class_elements, attributes, foreign_keys_map, pk_map = parse_json_model(data)

        # Build adjacency list for topological sort, so references come after the referenced tables
        # Edge: if A references B, then A depends on B
        adjacency_list = collections.defaultdict(list)
        in_degree = {cid: 0 for cid in class_elements}

        for cid, fk_list in foreign_keys_map.items():
            for _, target_id in fk_list:
                adjacency_list[target_id].append(cid)
                in_degree[cid] += 1

        # Topological sort
        queue = collections.deque(cid for cid, deg in in_degree.items() if deg == 0)
        sorted_ids = []
        while queue:
            node = queue.popleft()
            sorted_ids.append(node)
            for neighbor in adjacency_list[node]:
                in_degree[neighbor] -= 1
                if in_degree[neighbor] == 0:
                    queue.append(neighbor)

        # Generate SQL in topological order
        sql_statements = []
        for class_id in sorted_ids:
            class_data = class_elements[class_id]['data']
            class_name = class_data["name"]
            attr_ids = class_data.get("attributes", [])
            attr_defs = []

            # Regular columns (including PK)
            for attr_id in attr_ids:
                attr = attr_id if isinstance(attr_id, dict) else attributes.get(attr_id)
                if not attr:
                    continue
                col_name, col_type = parse_attribute(attr.get("name", ""))
                attr_defs.append((col_name, col_type))

            # Foreign-key columns with the same type as the referenced table's PK
            fk_list = foreign_keys_map.get(class_id, [])
            fk_constraints = []
            for role, target_id in fk_list:
                target_pk_name, target_pk_type = pk_map.get(target_id, ("id", "INTEGER"))
                attr_defs.append((role, target_pk_type))
                target_name = class_elements[target_id]['data']["name"]
                fk_constraints.append(
                    f'FOREIGN KEY("{role}") REFERENCES "{target_name}"("{target_pk_name}") '
                    f'ON UPDATE CASCADE ON DELETE SET NULL'
                )

            # Build CREATE TABLE statement
            
            pk_name, pk_type = pk_map.get(class_id, (None, None))

            lines = [f'CREATE TABLE "{class_name.replace(" ", "")}" (']
            for col_name, col_type in attr_defs:
                if pk_name and col_name == pk_name:
                    if pk_type.lower() == 'auto':
                        lines.append(f'  "{col_name.replace(" ", "")}" INTEGER PRIMARY KEY AUTOINCREMENT,')
                    else:
                        lines.append(f'  "{col_name.replace(" ", "")}" {col_type} PRIMARY KEY,')
                else:
                    lines.append(f'  "{col_name.replace(" ", "")}" {col_type},')

            # Add foreign-key constraints
            for constraint in fk_constraints:
                lines.append(f'  {constraint},')
            #if pk_name:
                #lines.append("  PRIMARY KEY(\"" + pk_name.replace(",", '","') + "\"),")
            if lines[-1].endswith(','):
                lines[-1] = lines[-1][:-1]
            lines.append(');')
            sql_statements.append('\n'.join(lines))

        return '\n'.join(sql_statements)
    except Exception as e:
        print(f"Error in extract_tables: {e}")
        raise e


def format_sql(sql: str) -> str:
    # Normalize whitespace
    sql = re.sub(r'\s+', ' ', sql).strip()

    # Keywords that should start on a new line (with optional indentation)
    newline_keywords = [
        r'\bSELECT\b', r'\bFROM\b', r'\bWHERE\b', r'\bGROUP BY\b', r'\bHAVING\b',
        r'\bORDER BY\b', r'\bLIMIT\b', r'\bOFFSET\b', r'\bJOIN\b', r'\bINNER JOIN\b',
        r'\bLEFT JOIN\b', r'\bRIGHT JOIN\b', r'\bFULL JOIN\b', r'\bOUTER JOIN\b',
        r'\bON\b', r'\bUNION\b', r'\bVALUES\b', r'\bSET\b', r'\bAND\b', r'\bOR\b',
        r'\bCREATE\b',r'\bFOREIGN\b',r'\bPRIMARY\b'
    ]

    # Keywords that should be followed by a tab indentation
    indent_keywords = [r'\bAND\b', r'\bOR\b', r'\bON\b']

    # Add line breaks before main keywords
    for kw in newline_keywords:
        sql = re.sub(f'(?i) {kw} ', lambda m: f'\n{m.group(0).strip()} ', sql)

    # Add tabs before indented keywords
    for kw in indent_keywords:
        sql = re.sub(f'(?i)^({kw})', r'\t\1', sql, flags=re.MULTILINE)

    return sql.strip()



def remove_nones_from_sqlresult(result:list):
    if result:
        return [tuple('' if v is None else v for v in row) for row in result]
    return []